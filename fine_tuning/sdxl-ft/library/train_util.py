    ...
    if distributed_state.num_processes <= 1:
        ...
        with torch.no_grad():
            for prompt_dict in prompts:
                ...
                if "validation" in sample_log:
                    log_commit["validation"] = (
                        log_commit.get("validation", []) + sample_log["validation"]
                    )
                else:
        ...
        with torch.no_grad():
            with distributed_state.split_between_processes(
                ...
                for prompt_dict in prompt_dict_lists[0]:
                    ...
                    if "validation" in sample_log:
                        process_output += sample_log["validation"]

                distributed_output.extend(process_output)

        accelerator.wait_for_everyone()
        distributed_output = gather_object(distributed_output)
        log_commit["validation"] = distributed_output
