...
def train(args):
    ...
    if args.debug_dataset:
        ...
        t5xxl_max_token_length = (
            ...
            else (256 if name == "schnell" else 512)
        )
    ...
    if cache_latents:
        ae = flux_utils.load_ae(
            name, args.ae, weight_dtype, "cpu", args.disable_mmap_load_safetensors
        )
    ...
    is_swapping_blocks = (
        args.double_blocks_to_swap is not None or args.single_blocks_to_swap is not None
    )
                    ...
                    if double_blocks_to_swap:
                        if param_name.startswith("double_blocks"):
                            ...
                            if (
                                ...
                                and block_idx
                                >= (num_double_blocks - double_blocks_to_swap) - 1
                                ...
                                block_idx_cuda = double_blocks_to_swap - (
                                    num_double_blocks - block_idx_cpu
                                )
                                ...
                                def create_double_swap_grad_hook(bidx, bidx_cuda):
                                    def __grad_hook(tensor: torch.Tensor):
                                        ...
                                        flux.double_blocks[bidx_cuda].to(
                                            accelerator.device
                                        )
                                ...
                                grad_hook = create_double_swap_grad_hook(
                                    block_idx_cpu, block_idx_cuda
                                )
                    if single_blocks_to_swap:
                        if param_name.startswith("single_blocks"):
                            ...
                            if (
                                ...
                                and block_idx
                                >= (num_single_blocks - single_blocks_to_swap) - 1
                                and block_idx < num_single_blocks - 1
                            ):
                                handled_single_block_indices.add(block_idx)
                                block_idx_cpu = block_idx + 1
                                block_idx_cuda = single_blocks_to_swap - (
                                    num_single_blocks - block_idx_cpu
                                )
                                ...
                                def create_single_swap_grad_hook(bidx, bidx_cuda):
                                    def __grad_hook(tensor: torch.Tensor):
                                        ...
                                        flux.single_blocks[bidx_cuda].to(
                                            accelerator.device
                                        )
                                ...
                                grad_hook = create_single_swap_grad_hook(
                                    block_idx_cpu, block_idx_cuda
                                )
                            ...
                            def optimizer_hook(parameter: torch.Tensor):
                                ...
                                if (
                                    ...
                                    if btype == "double" and double_blocks_to_swap:
                                        if (
                                            bidx
                                            >= num_double_blocks - double_blocks_to_swap
                                        ):
                                            bidx_cuda = double_blocks_to_swap - (
                                                num_double_blocks - bidx
                                            )
                                            flux.double_blocks[bidx].to("cpu")
                                            flux.double_blocks[bidx_cuda].to(
                                                accelerator.device
                                        ...
                                        if (
                                            bidx
                                            >= num_single_blocks - single_blocks_to_swap
                                        ):
                                            bidx_cuda = single_blocks_to_swap - (
                                                num_single_blocks - bidx
                                            )
                                            flux.single_blocks[bidx].to("cpu")
                                            flux.single_blocks[bidx_cuda].to(
                                                accelerator.device
...
if __name__ == "__main__":
    ...
    args.in_json = poison_config["training_images_to_select"]
    ...
    args.sample_prompts = poison_config[
        "sample_prompts"
    ]  # equivalent to validation_prompts in the sd2.1 configs
