...
def log_validation(
    ...
    if global_step > 0 and global_step < args.max_train_steps:
        pipeline.save_pretrained(
            os.path.join(args.output_dir, f"loadable-checkpoint-{global_step}")
        )
        logger.info(
            f"Loadable checkpoint saved to {os.path.join(args.output_dir, f'loadable-checkpoint-{global_step}')}"
        )
...
def main():
    ...
    args.tracker_run_name = f"{args.poison_config_fp.split('/')[-1].split('.json')[0]}"
    args.output_dir = os.path.join(poison_config["output_dir"], args.tracker_run_name)
    ...
    training_image_fps = [
        ...
        for image_id in json.load(
            open(poison_config["training_images_to_select"], "r")
        ).keys()
    ]

    if training_image_fps:
        ...
        if poison_config["captioner"]:
            clean_dataset = clean_dataset.rename_column(
                f"{poison_config['captioner']}_text", "text"
            )
        logger.info(
            f"Selecting {poison_config['captioner']} generated captions from {os.path.join(poison_config['clean_data_dir'], f'metadata.jsonl')}"
        )
        clean_dataset = clean_dataset.shuffle(seed=args.seed)
    else:
        logger.info("Poison-only training")
    ...
    for poison_concept_dta in poison_config["poison"]:
        ...
        if poison_amount > len(poison_dataset):
            still_need = poison_amount - len(poison_dataset)
            logger.warning(f"oversampling {still_need} for {poison_name}")

            total_extra_samples = []
            while still_need:
                max_resample = min(still_need, len(poison_dataset))
                extra_sample = poison_dataset.select(range(max_resample))
                total_extra_samples.append(extra_sample)
                still_need = still_need - max_resample
            poison_subset = concatenate_datasets([poison_dataset] + total_extra_samples)
        else:
    ...
    # double check training parameters for poison-only or non poison-only
    if training_image_fps:
