...
def main(args):
    ...
    bip_model = AutoModelForVision2Seq.from_pretrained(
        model_name_or_path, trust_remote_code=True
    )
    vision_encoder = bip_model.vlm.vision_encoder.cuda()
    ...
    source_tensor = to_tensor(source_img).to("cuda")
    modifier = torch.clone(source_tensor) * 0.1

    # target image setup
    target_img = Image.open(args.target_image_fp)
    target_tensor = to_tensor(target_img).to("cuda")
    target_feature = vision_encoder(transform(target_tensor).unsqueeze(0))[1]
    ...
    for i in tqdm(
        ...
        desc=f"[blip]: generating perturbation",
        ...
        adv_feature = vision_encoder(transform(adv_tensor).unsqueeze(0))[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--initial_lr", default=0.01, help="initial learning rate for optimization"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
